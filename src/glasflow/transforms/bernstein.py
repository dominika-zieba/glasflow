...
def bernstein_basis_polynomial(k, n):
    ...
    def basis_polynomial(x):
        return binomial_coeff * torch.exp(k * torch.log(x) + (n - k) * torch.log(1 - x))
...
def log_bernstein_basis_polynomial(k, n):
    # returns a berstein basis polynomial b_{k,n}(x) = nCk x^k (1-x)^(n-k), nCk = n!/(k!(n-k)!), as a function
    # gamma(n) = (n-1)!
    log_binomial_coeff = torch.special.gammaln(n + 1) - (torch.special.gammaln(n - k + 1) + torch.special.gammaln(k + 1))
...
def bernstein_transform_log_derivative(x, alphas):
    ...
    log_bernstein_basis = [log_bernstein_basis_polynomial(torch.tensor(k), torch.tensor(n-1)) for k in range(n)]
    log_bernstein_basis_at_x = torch.stack([log_basis_polynomial(x) for log_basis_polynomial in log_bernstein_basis])
    coeffs = n*(alphas[:,1:] - alphas[:,:-1])      #(alpha_k - alpha_k-1) for k between 0 and n
    logdet = torch.logsumexp(log_bernstein_basis_at_x + torch.log(coeffs.T), dim = 0)
...
def get_increasing_alphas_nd(unconstrained_params, range_min=0, range_max=1):
    ...
    alphas = torch.concatenate(
        [torch.full((unconstrained_params.shape[0], 1), range_min), scaled_params],
        axis=-1,
    )
...
def bernstein_fwd(x, alphas):
    ...
    n = alphas.shape[-1] - 1  # degree of the bernstein polynomial
    bernstein_basis = [bernstein_basis_polynomial(torch.tensor(k), torch.tensor(n)) for k in range(n + 1)]
    basis_at_x = torch.stack([basis_polynomial(x) for basis_polynomial in bernstein_basis])
...
def bernstein_fwd_log(x, alphas):
    ...
    n = alphas.shape[-1] - 1  # degree of the bernstein polynomial
    log_basis_at_x = torch.stack([log_bernstein_basis_polynomial(torch.tensor(k), torch.tensor(n))(x) for k in range(n + 1)])
...
def bernstein_log_det(x, alphas):
    ...
    bernstein_basis_derivatives = [
        bernstein_basis_polynomial_derivative(torch.tensor(k), torch.tensor(n)) for k in range(n + 1)
]
    ...
    derivative_in_basis = torch.sum(torch.multiply(basis_derivative_at_x, alphas.T), axis = 0)
...
def bernstein_transform_inv(y, alphas):
    ...
    alphas_long = torch.tile(alphas, (n_points, 1, 1)) #the same alpha for every sample 
...
def bernstein_transform(
    ...
    unconstrained_alphas = unconstrained_alphas.reshape((inputs.shape[0], unconstrained_alphas.shape[-1]))
    ...
    alphas = get_increasing_alphas_nd(unconstrained_alphas, range_min=0, range_max=1)
